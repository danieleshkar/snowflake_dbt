with global_tenant_history_daily_agg_billing_tbl as (
    ...
    {{ ref('current_global_tenant_ltp_hwm_history')}} 
    WHERE
...
sltp_daily_itemized_billing_tbl as (
    select * from 
    {{ ref('sltp_daily_itemized_new_billing_tbl')}}
    -- WHERE
...
sltp_bill AS (
    ...
    SELECT
      ...
      MAX(CASE WHEN item = 'Email Protect' AND partner_pricing = false THEN quantity ELSE 0 END) as Email_Protect_of_Licenses,

      MAX(CASE WHEN item = 'Email Protect' AND partner_pricing = true THEN true ELSE false END) as Email_Protect_NFR,
      MAX(CASE WHEN item = 'Email Protect' AND partner_pricing = true THEN quantity ELSE 0 END) as Email_Protect_NFR_of_Licenses,
    
      -- Complete Protect
      MAX(CASE WHEN item = 'Complete Protect' AND partner_pricing = false THEN true ELSE false END) as Complete_Protect,
      MAX(CASE WHEN item = 'Complete Protect' AND partner_pricing = false THEN quantity ELSE 0 END) as Complete_Protect_of_Licenses,

      MAX(CASE WHEN item = 'Complete Protect' AND partner_pricing = true THEN true ELSE false END) as Complete_Protect_NFR,
      MAX(CASE WHEN item = 'Complete Protect' AND partner_pricing = true THEN quantity ELSE 0 END) as Complete_Protect_NFR_of_Licenses,
      
      -- Core
      MAX(CASE WHEN item = 'Core' AND partner_pricing = false THEN true ELSE false END) as Core,
      MAX(CASE WHEN item = 'Core' AND partner_pricing = false THEN quantity ELSE 0 END) as Core_of_Licenses,

      MAX(CASE WHEN item = 'Core' AND partner_pricing = true THEN true ELSE false END) as Core_NFR,
      MAX(CASE WHEN item = 'Core' AND partner_pricing = true THEN quantity ELSE 0 END) as Core_NFR_of_Licenses,
      
      -- IRONSCALES Protect
      MAX(CASE WHEN item = 'IRONSCALES Protect' AND partner_pricing = false THEN true ELSE false END) as Ironscales_Protect,
      MAX(CASE WHEN item = 'IRONSCALES Protect' AND partner_pricing = false THEN quantity ELSE 0 END) as Ironscales_Protect_of_Licenses,

      MAX(CASE WHEN item = 'IRONSCALES Protect' AND partner_pricing = true THEN true ELSE false END) as Ironscales_Protect_NFR,
      MAX(CASE WHEN item = 'IRONSCALES Protect' AND partner_pricing = true THEN quantity ELSE 0 END) as Ironscales_Protect_NFR_of_Licenses,
      ...
      MAX(CASE WHEN item = 'Phishing Simulation and Training' AND partner_pricing = false THEN quantity ELSE 0 END) as PST_of_Licenses,
      ...
      MAX(CASE WHEN item = 'Phishing Simulation and Training' AND partner_pricing = true THEN quantity ELSE 0 END) as PST_NFR_of_Licenses,
    
      -- Starter
      MAX(CASE WHEN item = 'Starter' THEN true ELSE false END) as Starter,
      MAX(CASE WHEN item = 'Starter' THEN quantity ELSE 0 END) as Starter_of_Licenses,
    
      -- Premium Content
      MAX(CASE WHEN item IN ('Habitu8') THEN true ELSE false END) as Premium_Content_Habitu8,
      MAX(CASE WHEN item IN ('Habitu8') THEN quantity ELSE 0 END) as Premium_Content_Habitu8_of_Licenses,

      MAX(CASE WHEN item IN ('NINJIO') THEN true ELSE false END) as Premium_Content_NINJIO,
      MAX(CASE WHEN item IN ('NINJIO') THEN quantity ELSE 0 END) as Premium_Content_NINJIO_of_Licenses,

      MAX(CASE WHEN item IN ('Cybermaniacs Videos') THEN true ELSE false END) as Premium_Content_Cybermaniacs,
      MAX(CASE WHEN item IN ('Cybermaniacs Videos') THEN quantity ELSE 0 END) as Premium_Content_Cybermaniacs_of_Licenses,
      
      -- Account Takeover
      MAX(CASE WHEN item = 'Account Takeover' THEN true ELSE false END) as Account_Takeover,
      MAX(CASE WHEN item = 'Account Takeover' THEN quantity ELSE 0 END) as Account_Takeover_of_Licenses,
    
      -- Incident Management
      MAX(CASE WHEN item = 'Incident Management' THEN true ELSE false END) as Incident_Management,
      MAX(CASE WHEN item = 'Incident Management' THEN quantity ELSE 0 END) as Incident_Management_of_Licenses,
    
      -- Multi Tenant
      MAX(CASE WHEN item = 'Multi Tenant' THEN true ELSE false END) as Multi_Tenant,
      MAX(CASE WHEN item = 'Multi Tenant' THEN quantity ELSE 0 END) as Multi_Tenant_of_Licenses,
    
      -- S&T Bundle
      MAX(CASE WHEN item = 'S&T Bundle' THEN true ELSE false END) as ST_Bundle,
      MAX(CASE WHEN item = 'S&T Bundle' THEN quantity ELSE 0 END) as ST_Bundle_of_Licenses,
    
      -- S&T Plus Bundle
      MAX(CASE WHEN item = 'S&T Bundle Plus' THEN true ELSE false END) as ST_Bundle_Plus,
      MAX(CASE WHEN item = 'S&T Bundle Plus' THEN quantity ELSE 0 END) as ST_Bundle_Plus_of_Licenses,
    
      -- Security Awareness Training
      MAX(CASE WHEN item = 'Security Awareness Training' THEN true ELSE false END) as Security_Awareness_Training,
      MAX(CASE WHEN item = 'Security Awareness Training' THEN quantity ELSE 0 END) as SAT_of_Licenses,

      -- Security Awareness Training
      MAX(CASE WHEN item = 'SAT Suite' AND partner_pricing = false THEN true ELSE false END) as Security_Awareness_Training_Suite,
      MAX(CASE WHEN item = 'SAT Suite' AND partner_pricing = false THEN quantity ELSE 0 END) as SAT_Suite_of_Licenses,

      MAX(CASE WHEN item = 'SAT Suite' AND partner_pricing = true THEN true ELSE false END) as Security_Awareness_Training_Suite_NFR,
      MAX(CASE WHEN item = 'SAT Suite' AND partner_pricing = true THEN quantity ELSE 0 END) as SAT_Suite_NFR_of_Licenses, 

    --   --DMARC ---
    --   MAX(CASE WHEN item = 'DMARC' THEN true ELSE false END) as DMARC_Management,
    --   MAX(CASE WHEN item = 'DMARC' THEN quantity ELSE 0 END) as DMARC_Management_of_Licenses
      --core--
      MAX(CASE WHEN item = 'DMARC Core Management' THEN true ELSE false END) as DMARC_Core_Management,
      MAX(CASE WHEN item = 'DMARC Core Management' THEN quantity ELSE 0 END) as DMARC_Core_Management_of_Licenses,
      --pro--
      MAX(CASE WHEN item = 'DMARC Pro' THEN true ELSE false END) as DMARC_Pro_Management,
      MAX(CASE WHEN item = 'DMARC Pro' THEN quantity ELSE 0 END) as DMARC_Pro_Management_of_Licenses,
      --premium--
      MAX(CASE WHEN item = 'DMARC Premium' THEN true ELSE false END) as DMARC_Premium_Management,
      MAX(CASE WHEN item = 'DMARC Premium' THEN quantity ELSE 0 END) as DMARC_Premium_Management_of_Licenses
...
left join global_tenant_history gh on 
                                        g.hwm_date = gh.record_date
                                        -- g.record_date = gh.record_date 
...
WHERE
REGEXP_SUBSTR(g.tenant_global_id, '[A-Za-z]+') = 'CA'
